...
router.get("/get-partner-credentials", (req, res) => {
  const partnerUsername = process.env.PARTNER_USERNAME;
  const partnerPassword = process.env.PARTNER_PASSWORD; // Fetch the password from the backend .env file

  // Optionally, you could implement some basic security checks here
  // For example, ensure that the request is coming from a trusted source
  res.json({ username: partnerUsername, password: partnerPassword });
});
